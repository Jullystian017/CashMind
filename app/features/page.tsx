...
const ScoreVisual = () => (
    <div className="w-full h-full min-h-[320px] sm:min-h-[400px] flex items-center justify-center bg-gradient-to-br from-blue-50 to-white relative overflow-hidden rounded-[2.5rem] border border-blue-100 shadow-xl">
        <div className="relative flex flex-col items-center scale-75 sm:scale-100">
            {/* Outer glowing rings */}
            <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute w-60 h-60 sm:w-72 sm:h-72 rounded-full border border-dashed border-blue-300/50"
            />
            <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
                className="absolute w-64 h-64 sm:w-80 sm:h-80 rounded-full border border-blue-100"
            />

            {/* The Score Circle */}
            <div className="relative w-48 h-48 sm:w-56 sm:h-56 rounded-full bg-white shadow-2xl flex flex-col items-center justify-center border-8 border-blue-50 z-10">
                <p className="text-gray-400 font-semibold uppercase tracking-widest text-[10px] sm:text-xs mb-1">Health Score</p>
                <div className="flex items-start">
                    <motion.span
                        ...
                        className="text-6xl sm:text-7xl font-semibold text-blue-600 tracking-tighter"
                    >
                        85
                    </motion.span>
                </div>
                <div className="mt-2 bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold flex items-center gap-1">
...
const AnalyticsVisual = () => (
    <div className="w-full h-full min-h-[320px] sm:min-h-[400px] flex items-center justify-center bg-gradient-to-br from-indigo-50 to-white relative overflow-hidden rounded-[2.5rem] border border-indigo-100 shadow-xl p-4 sm:p-8">
        <div className="w-full h-full bg-white rounded-3xl shadow-sm border border-gray-50 flex flex-col justify-end relative overflow-hidden p-4 sm:p-6 z-10">
            {/* Grid background */}
            ...
            <div className="flex justify-between items-center mb-auto relative z-10">
                <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-900">Spending Overview</h4>
                    <p className="text-[10px] sm:text-xs text-gray-400">Past 6 months</p>
                </div>
                <div className="bg-indigo-50 text-indigo-600 p-1.5 sm:p-2 rounded-xl">
                    <PieChart className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
            </div>

            {/* Animated Bar Chart */}
            <div className="flex items-end justify-between h-32 sm:h-48 gap-1.5 sm:gap-2 relative z-10 mt-4 sm:mt-6 pt-4 border-t border-dashed border-gray-200">
                {[40, 70, 45, 90, 65, 100].map((height, i) => (
                    <div key={i} className="w-full flex flex-col items-center gap-1 sm:gap-2">
                        <motion.div
                        ...
                        />
                        <span className="text-[8px] sm:text-[10px] font-semibold text-gray-400">M{i + 1}</span>
                    </div>
                ))}
            </div>
            ...
            <motion.div
                ...
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold shadow-2xl flex items-center gap-2"
            >
...
const BudgetVisual = () => (
    <div className="w-full h-full min-h-[320px] sm:min-h-[400px] flex items-center justify-center bg-gradient-to-br from-cyan-50 to-white relative overflow-hidden rounded-[2.5rem] border border-cyan-100 shadow-xl p-4 sm:p-8">
        <div className="w-full max-w-sm flex flex-col gap-3 sm:gap-4 relative z-10 scale-90 sm:scale-100">
            {/* Category Cards */}
            {[
                ...
                <motion.div
                    ...
                    className="bg-white p-3 sm:p-4 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow"
                >
                    <div className="flex justify-between items-center mb-2 sm:mb-3">
                        <div className="flex items-center gap-2 sm:gap-3">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center ${cat.lightBg} ${cat.lightText}`}>
                                <Activity className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                            </div>
                            <span className="font-semibold text-gray-900 text-sm sm:text-base">{cat.name}</span>
                        </div>
                        <span className="text-[10px] sm:text-sm font-semibold text-gray-500">${cat.used} <span className="text-gray-300">/ ${cat.limit}</span></span>
                    </div>
                    {/* Progress Bar */}
            ...
            <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: 1 }}
                className="absolute -bottom-4 -right-4 bg-red-50 border border-red-100 p-3 sm:p-4 rounded-xl shadow-xl flex items-center gap-2 sm:gap-3"
            >
                ...
                <p className="text-[10px] sm:text-xs font-semibold text-red-900">Food budget nearing limit!</p>
            </motion.div>
        </div>
                            ...
                            <div key={idx} className={`flex flex-col ${feature.reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-16 lg:gap-24 items-center`}>
                                ...
                                >
                                    <div>
                                        ...
                                        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-gray-900 tracking-tight leading-tight mb-4">
                                            {feature.title}
                                        </h2>
