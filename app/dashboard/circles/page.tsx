      ...
      <div className="space-y-8 pb-24 @container" suppressHydrationWarning>
        ...
        <div className="flex flex-col @sm:flex-row @sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <button onClick={() => { setSelectedCircleId(null); setCircleDetail(null) }} className="p-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-600 transition-all shrink-0">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-2xl shrink-0">{circle.emoji}</span>
                <h2 className="text-xl font-bold text-gray-900 tracking-tight truncate">{circle.name}</h2>
              </div>
              <p className="text-xs text-gray-500 mt-0.5">{members.length} members</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => copyCode(circle.invite_code)} className="flex-1 @sm:flex-none flex items-center justify-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 text-[10px] font-bold text-gray-600 transition-all h-10">
              {copied ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
              {circle.invite_code}
            </button>
            <button onClick={() => setIsAddExpenseOpen(true)} className="flex-1 @sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl text-white text-[10px] font-bold shadow-lg shadow-blue-500/20 transition-all h-10">
          ...
          <div className="grid grid-cols-1 @sm:grid-cols-2 gap-3 @sm:gap-4">
            {insights.map((insight: string, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                className="bg-white p-4 @sm:p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
                <div className="flex gap-3">
                  <div className="w-8 h-8 bg-blue-50 rounded-xl flex items-center justify-center shrink-0">
                    <Sparkles className="w-4 h-4 text-blue-600" />
                  </div>
                  <p className="text-xs @sm:text-sm text-gray-700 leading-relaxed font-medium">{insight}</p>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          {isAddExpenseOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ y: 50, opacity: 0, scale: 0.95 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 50, opacity: 0, scale: 0.95 }}
                className="bg-white rounded-[32px] p-5 w-full max-w-md shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base font-bold text-gray-900">{t("circles.addExpense")}</h3>
                  <button onClick={() => setIsAddExpenseOpen(false)} className="p-1.5 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors"><X className="w-4 h-4" /></button>
                </div>
                <div className="space-y-3">
                  <div>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1.5">{t("circles.category")}</label>
                      ...
                      {CATEGORIES.map((c) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "flex flex-col items-center justify-center gap-1 p-2 rounded-xl text-[9px] font-bold transition-all border text-center leading-tight h-16",
                              isSelected ? "bg-blue-50 border-blue-200 text-blue-700 shadow-sm" : "bg-white border-gray-100 text-gray-600 hover:bg-gray-50"
                            )}
                          >
                            <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center")} style={{ color: config.color, backgroundColor: `${config.color}15` }}>
                  ...
                  <button onClick={handleAddExpense} disabled={!expDesc || !expAmount}
                    className="w-full mt-2 py-3.5 bg-blue-600 text-white rounded-2xl font-bold text-sm hover:bg-blue-700 disabled:bg-gray-100 disabled:text-gray-400 transition-all shadow-lg shadow-blue-500/20">
    ...
    <div className="space-y-8 pb-24 @container" suppressHydrationWarning>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative z-10 flex flex-col @sm:flex-row items-center gap-4 @sm:gap-6">
          <div className="p-4 bg-white/10 backdrop-blur-xl rounded-2xl shadow-sm border border-white/20 shrink-0">
            <Users className="w-8 h-8 text-blue-100" />
          </div>
          <div className="text-center @sm:text-left">
            <h3 className="text-xl font-bold text-white">{t("circles.title")}</h3>
            <p className="text-xs @sm:text-sm text-blue-100/70 mt-1 max-w-lg">
              {t("circles.subtitle")}
            </p>
          </div>
        </div>
